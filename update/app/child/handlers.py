...
def _build_prompt(
    ...
    Language locking: the customer's language is detected ONCE (from the
    ORIGINAL message, before any translation) and (a) pinned inside the
    system prompt, (b) repeated as the very last line of the prompt (recency
    strongly increases compliance). History labels are language-neutral
    emojis so they cannot bias the reply tongue.

    When the customer's message was auto-translated into English
    (``original_text`` given and different), the prompt says so explicitly —
    otherwise a model that sees an English question might ignore the
    "answer in {user_language}" rule and think the reminder is stale.
    """
    if user_language is None:
        user_language = detect_language(user_text)
    translated = bool(original_text) and original_text != user_text
    ...
    if translated:
        parts.append(
            "👤 پیام جدید مشتری (new customer message — auto-translated into English):\n"
            + user_text
            + "\n(پیام اصلی مشتری برای لحن و اشاره‌ها / original customer message:\n"
            + (original_text or "")
            + ")"
        )
    else:
    ...
    if user_language:
        ...
        if translated:
            parts.append(
                f"⚠️ REMINDER: The customer writes in {user_language} ({fa}); their message "
                f"was translated into English only so you understand it better. "
                f"Your answer MUST still be written ONLY in {user_language} — "
                f"هیچ جمله‌ای به زبان دیگری ننویس."
            )
        else:
            parts.append(
...
async def _reply_with_ai(message: Message, bot: Bot, bot_row, text: str) -> None:
    ...
    Translation pipeline (both directions, we control the language ourselves):
    1. The customer's language is detected from the ORIGINAL message.
    2. If it is anything other than English, the question is translated into
       English and the ENGLISH text is sent to the model (models understand
       and answer English most reliably). The original text rides along in
       the prompt for tone/reference.
    3. Whatever the model answers, after the reply arrives we make sure the
       customer receives it in THEIR language: if the reply's language does
       not match the customer's language, we translate the reply ourselves.
    """
    is_pro = await _owner_is_pro(bot_row["owner_id"])
    ...
    try:
        # ---- step 1: translate a non-English question into English
        prompt_text = text
        original_text: str | None = None
        if user_language and user_language != "English":
            try:
                translated_q = await translate_text(text, "English")
            except AIError:
                translated_q = None  # fall back to the original text
            if translated_q and translated_q.strip() and translated_q.strip() != text:
                prompt_text = translated_q.strip()
                original_text = text

        # ---- step 2: ask the model (English question, language still pinned)
    ...
    # ---- step 3: language enforcement — the customer always gets THEIR tongue
    if user_language:
